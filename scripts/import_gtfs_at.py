#!/usr/bin/env python3
"""
WoW TRENES - Importador GTFS Austria -> SQLite
Lee ~/Downloads/Wow trains Austria/ y genera assets/gtfs_at.db

Fuente: ÖBB — CC BY 4.0
  URL: https://data.oebb.at/de/datensaetze~soll-fahrplan-gtfs~
  Requiere aceptar TOS en la web. Click 'Ich akzeptiere die Nutzungsbedingungen' → Download.

Incluye: ÖBB nacional — Wien, Salzburg, Innsbruck, Graz, Linz

Resultado esperado: ~1.000 estaciones, ~100k stop_times, ~20-40 MB
"""
import sqlite3, csv, sys, time, shutil
from pathlib import Path

_POSSIBLE_DIRS = [
    Path.home() / "Downloads" / "Wow trains Austria",
    Path.home() / "Downloads" / "Wow trains Austria ",
    Path.home() / "Downloads" / "AUSTRIA",
    Path.home() / "Downloads" / "Austria",
    Path.home() / "Downloads" / "oebb",
    Path.home() / "Downloads" / "OBB",]
GTFS_DIR   = next((d for d in _POSSIBLE_DIRS if d.exists()), _POSSIBLE_DIRS[0])
OUTPUT_DIR = Path(__file__).parent.parent / "assets"
OUTPUT_DB  = OUTPUT_DIR / "gtfs_at.db"
COUNTRY    = "AT"
...
def setup(conn):
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS stops (
            ...
            country_code TEXT DEFAULT 'AT', location_type INTEGER DEFAULT 0,
            parent_station TEXT);
...
def main():
    print(f"\nWoW TRENES - Importador GTFS Austria (ÖBB)")
    print("=" * 55)
    ...
    if not GTFS_DIR.exists():
        ...
        print("\nDescarga:")
        print("  https://data.oebb.at/de/datensaetze~soll-fahrplan-gtfs~")
        print("  Requiere aceptar TOS en la web. Click 'Ich akzeptiere die Nutzungsbedingungen' → Download.")
        print(f"  Descomprime en: ~/Downloads/Wow trains Austria/")
        sys.exit(1)
    ...
    tmp_db = Path("/tmp/gtfs_at_build.db")
    ...
    try:
        ...
        conn.executemany("INSERT OR IGNORE INTO agency VALUES (?,?,?,?)",
            [(r.get("agency_id",""), r.get("agency_name",""),
              r.get("agency_url",""), r.get("agency_timezone","Europe/Vienna"))
             for r in rows])
        conn.commit()
        ...
        print(f"\nAustria lista con datos de ÖBB.")
