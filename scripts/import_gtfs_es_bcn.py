#!/usr/bin/env python3
"""
WoW TRENES - Importador GTFS Barcelona Metro (TMB) → SQLite
Genera assets/gtfs_es_bcn.db

══════════════════════════════════════════════════════════════════
FUENTE (requiere registro gratuito en API TMB):
──────────────────────────────────────────────────────────────────
TMB (Transports Metropolitans de Barcelona)
  Portal API: https://developer.tmb.cat/
  Endpoint:   GET /v1/gtfs/static  (requiere API key gratuita)
  Descarga manual: buscar "GTFS TMB" en https://opendata-ajuntament.barcelona.cat
  Carpeta destino: ~/Downloads/Barcelona Metro/
  Cubre: L1-L12 · ~150 estaciones · TMB + FGC parcial

ALTERNATIVA sin registro:
  Transitland: https://www.transit.land/feeds/f-sp3-tmb
  → Download latest → descomprimir en ~/Downloads/Barcelona Metro/
...
_POSSIBLE_DIRS = [
    Path.home() / "Downloads" / "Barcelona Metro",
    Path.home() / "Downloads" / "barcelona_metro",
    Path.home() / "Downloads" / "TMB",
    Path.home() / "Downloads" / "tmb",
    Path.home() / "Downloads" / "BCN Metro",
    Path.home() / "Downloads" / "ES_BCN",
]
...
def main():
    print("\nWoW TRENES - Importador GTFS Barcelona Metro (TMB)")
    print("=" * 52)

    if not GTFS_DIR.exists():
        print(f"\nERROR: Carpeta no encontrada. Probé:")
        for d in _POSSIBLE_DIRS: print(f"  {d}")
        print("\nOpciones para obtener el feed:")
        print("  1. Registrarse gratis en https://developer.tmb.cat/")
        print("     Descargar GTFS estático → descomprimir en ~/Downloads/Barcelona Metro/")
        print("  2. Via Transitland (sin registro):")
        print("     https://www.transit.land/feeds/f-sp3-tmb")
        print("     → Downloads → latest GTFS → ~/Downloads/Barcelona Metro/")
        sys.exit(1)
    ...
    conn.executemany("INSERT OR IGNORE INTO agency VALUES (?,?,?,?)",
        [(r.get("agency_id","tmb"), r.get("agency_name","TMB Barcelona"),
          r.get("agency_url","https://www.tmb.cat"),
          r.get("agency_timezone","Europe/Madrid")) for r in rows])
    conn.commit()

    # Routes — incluir todo (metro, bus, etc. del feed TMB)
    rows = read_csv(GTFS_DIR / "routes.txt")
    routes = []
    for r in rows:
        rt = str(r.get("route_type","1")).strip()
        routes.append((
            r["route_id"], r.get("agency_id","tmb"),
            r.get("route_short_name",""), r.get("route_long_name",""),
            int(rt or "1"), r.get("route_color",""), "Barcelona TMB"
        ))
