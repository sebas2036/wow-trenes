#!/usr/bin/env python3
"""
Importa GTFS real CP (Comboios de Portugal) → assets/gtfs_portugal.db

Uso:
  python3 scripts/import_gtfs_portugal_real.py <ruta_al_zip>

Ejemplo:
  python3 scripts/import_gtfs_portugal_real.py ~/Downloads/gtfs\ \(1\).zip
"""

import csv, io, os, sqlite3, sys, time, zipfile

SRC_PATH = sys.argv[1] if len(sys.argv) > 1 else 'gtfs (1).zip'
OUT_DB   = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'gtfs_portugal.db'))
...
log(f'=== Importando GTFS Portugal (CP) desde {SRC_PATH} ===')
...
c2.execute("""
    ...
    WHERE (s_o.stop_name LIKE '%Lisboa%' OR s_o.stop_name LIKE '%Porto%')
      AND st_o.departure_time >= '08:00:00'
    LIMIT 5
""")
print('Sample Lisboa/Porto:', c2.fetchall())
con2.close()
