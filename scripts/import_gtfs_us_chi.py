#!/usr/bin/env python3
"""
WoW TRENES - Importador GTFS Chicago CTA L → SQLite
Genera assets/gtfs_us_chi.db

══════════════════════════════════════════════════════════════════
FUENTE (gratuita, sin registro):
──────────────────────────────────────────────────────────────────
Chicago Transit Authority (CTA) — L Train (Elevated)
  URL directa: https://www.transitchicago.com/downloads/sch_data/
               google_transit.zip
  Carpeta:     ~/Downloads/Chicago CTA/
  Cubre: 8 líneas (Red, Blue, Green, Brown, Purple, Yellow, Pink, Orange)
         ~145 estaciones · Loop + extensiones
...
_POSSIBLE_DIRS = [
    Path.home() / "Downloads" / "Chicago CTA",
    Path.home() / "Downloads" / "CTA",
    Path.home() / "Downloads" / "chicago_cta",
    Path.home() / "Downloads" / "US_CHI",
    Path.home() / "Downloads" / "Chicago",
]
GTFS_DIR   = next((d for d in _POSSIBLE_DIRS if d.exists()), _POSSIBLE_DIRS[0])
OUTPUT_DIR = Path(__file__).parent.parent / "assets"
OUTPUT_DB  = OUTPUT_DIR / "gtfs_us_chi.db"
COUNTRY    = "US"
MAX_ST     = 200_000
MAX_CD     = 30_000
...
def main():
    print("\nWoW TRENES - Importador GTFS Chicago CTA L")
    print("=" * 50)
    ...
    if not GTFS_DIR.exists():
        ...
        print("     https://www.transitchicago.com/downloads/sch_data/google_transit.zip")
        print("  2. Descomprime en: ~/Downloads/Chicago CTA/")
        sys.exit(1)
    ...
    tmp_db = Path("/tmp/gtfs_us_chi_build.db")
    ...
    conn.executemany("INSERT OR IGNORE INTO agency VALUES (?,?,?,?)",
        [(r.get("agency_id","CTA"), r.get("agency_name","Chicago Transit Authority"),
          r.get("agency_url","https://www.transitchicago.com"),
          r.get("agency_timezone","America/Chicago")) for r in rows])
    conn.commit()

    # Routes — CTA L usa route_type 1 (subway)
    rows = read_csv(GTFS_DIR / "routes.txt")
    routes = []
    for r in rows:
        rt = str(r.get("route_type","1")).strip()
        routes.append((
            r["route_id"], r.get("agency_id","CTA"),
            r.get("route_short_name",""), r.get("route_long_name",""),
            int(rt or "1"), r.get("route_color",""), "Chicago CTA L"
        ))
    ...
    print(f"\nCTA L: 8 líneas · ~145 estaciones")
    print("  Red Line, Blue Line, Green Line, Brown Line")
    print("  Purple Line, Yellow Line, Pink Line, Orange Line")
