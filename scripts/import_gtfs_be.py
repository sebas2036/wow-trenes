#!/usr/bin/env python3
"""
WoW TRENES - Importador GTFS Bélgica -> SQLite
Lee ~/Downloads/Wow trains Belgium/ y genera assets/gtfs_be.db

Fuente: SNCB/NMBS — libre, sin restricciones (iRail)
  URL: http://gtfs.irail.be/nmbs/nmbs-latest.zip
  Descarga directa — mantenido por iRail (comunidad open source).

Incluye: SNCB/NMBS nacional — Bruxelles, Brugge, Gent, Antwerpen, Liège

Resultado esperado: ~2.700 estaciones, ~200k stop_times, ~30-50 MB
"""
import sqlite3, csv, sys, time, shutil
from pathlib import Path

_POSSIBLE_DIRS = [
    Path.home() / "Downloads" / "Wow trains Belgium",
    Path.home() / "Downloads" / "Wow trains Belgium ",
    Path.home() / "Downloads" / "BELGIUM",
    Path.home() / "Downloads" / "Belgium",
    Path.home() / "Downloads" / "BELGICA",
    Path.home() / "Downloads" / "nmbs",
    Path.home() / "Downloads" / "sncb",]
GTFS_DIR   = next((d for d in _POSSIBLE_DIRS if d.exists()), _POSSIBLE_DIRS[0])
OUTPUT_DIR = Path(__file__).parent.parent / "assets"
OUTPUT_DB  = OUTPUT_DIR / "gtfs_be.db"
COUNTRY    = "BE"
...
def setup(conn):
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS stops (
            ...
            country_code TEXT DEFAULT 'BE', location_type INTEGER DEFAULT 0,
            parent_station TEXT);
...
def main():
    print(f"\nWoW TRENES - Importador GTFS Bélgica (SNCB/NMBS)")
    print("=" * 55)
    ...
    if not GTFS_DIR.exists():
        ...
        print("\nDescarga:")
        print("  http://gtfs.irail.be/nmbs/nmbs-latest.zip")
        print("  Descarga directa — mantenido por iRail (comunidad open source).")
        print(f"  Descomprime en: ~/Downloads/Wow trains Belgium/")
        sys.exit(1)
    ...
    tmp_db = Path("/tmp/gtfs_be_build.db")
    ...
    try:
        ...
        conn.executemany("INSERT OR IGNORE INTO agency VALUES (?,?,?,?)",
            [(r.get("agency_id",""), r.get("agency_name",""),
              r.get("agency_url",""), r.get("agency_timezone","Europe/Brussels"))
             for r in rows])
        conn.commit()
        ...
        print(f"\nBélgica lista con datos de SNCB/NMBS.")
