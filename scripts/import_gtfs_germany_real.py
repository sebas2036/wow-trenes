#!/usr/bin/env python3
"""
Importa GTFS real DB (Deutsche Bahn / GTFS_Fahrplan_2026) → assets/gtfs_germany.db

Fuente: GTFS Deutschland (gtfs.de / DB Fernverkehr + Regional)
Uso:
  python3 scripts/import_gtfs_germany_real.py <ruta_al_zip>

Ejemplo:
  python3 scripts/import_gtfs_germany_real.py ~/Downloads/GTFS_Fahrplan_2026.zip

Filtra route_type IN (2,100,101,102,103,104,105,107) — ICE, IC, RE, RB, S-Bahn
"""

import csv, io, os, sqlite3, sys, time, zipfile

SRC_PATH = sys.argv[1] if len(sys.argv) > 1 else 'GTFS_Fahrplan_2026.zip'
OUT_DB   = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'gtfs_germany.db'))
RAIL_TYPES   = {2, 100, 101, 102, 103, 104, 105, 107, 106, 109}
COUNTRY_CODE = 'DE'
...
log(f'=== Importando GTFS Alemania (DB) desde {SRC_PATH} ===')
...
c2.execute("""
    ...
    WHERE (s_o.stop_name LIKE '%Berlin%' OR s_o.stop_name LIKE '%Frankfurt%' OR s_o.stop_name LIKE '%München%')
      AND st_o.departure_time >= '08:00:00'
    LIMIT 5
""")
print('Sample Berlin/Frankfurt/München:', c2.fetchall())
