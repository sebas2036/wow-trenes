#!/usr/bin/env python3
"""
Importa GTFS real NS Netherlands → assets/gtfs_netherlands.db
Fuente: gtfs-openov-nl (transporte nacional holandés)

Uso:
  python3 scripts/import_gtfs_netherlands_real.py <ruta_al_zip>

Ejemplo:
  python3 scripts/import_gtfs_netherlands_real.py ~/Downloads/gtfs-openov-nl.zip

Filtra: route_type IN (2,100,101,102,103,104,105,107) — IC, Intercity Direct, Sprinter
Excluye: bus (700+), tram (900+), metro (401), barco (1000+)
"""

import csv, io, os, sqlite3, sys, time, zipfile

SRC_PATH = sys.argv[1] if len(sys.argv) > 1 else 'gtfs-openov-nl.zip'
OUT_DB   = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'gtfs_netherlands.db'))

# NS trenes principales
RAIL_TYPES = {2, 100, 101, 102, 103, 104, 105, 107}

def log(msg): print(f'[{time.strftime("%H:%M:%S")}] {msg}', flush=True)

def find_and_read(zf, base):
    names = zf.namelist()
    match = next((n for n in names if n.split('/')[-1].lower() == base.lower()), None)
    if not match:
        raise FileNotFoundError(f'{base} not in zip. Files: {[n for n in names[:20]]}')
    data = zf.read(match).decode('utf-8-sig')
    return csv.DictReader(io.StringIO(data)), match

log(f'=== Importando GTFS Países Bajos (NS) desde {SRC_PATH} ===')
...
names = zf.namelist()
log(f'Archivos ({len(names)} total): {[n for n in names if not n.endswith("/")][:15]}')

# ── 1. Routes ─────────────────────────────────────────────────────────────────
log('Leyendo routes.txt...')
rail_routes = {}
type_counts = {}
rd, _ = find_and_read(zf, 'routes.txt')
for row in rd:
    try:
    ...
    if rt in RAIL_TYPES:
        # Para NS: route_short_name suele ser "IC", "SPR", "ICD", "NS", etc.
...
log(f'  route_type counts (top): {sorted(type_counts.items())[:10]}')
log(f'  {len(rail_routes)} rutas de tren')

if not rail_routes:
    log('  WARN: sin rutas con route_type de tren. Importando route_type=2 solamente...')
    rd2, _ = find_and_read(zf, 'routes.txt')
    for row in rd2:
    ...
    log(f'  {len(rail_routes)} rutas (route_type=2)')

# ── 2. Trips ──────────────────────────────────────────────────────────────────
log('Leyendo trips.txt...')
rail_trips = {}
rd, _ = find_and_read(zf, 'trips.txt')
for row in rd:
...
# ── 3. Stop_times — streaming ─────────────────────────────────────────────────
log('Streaming stop_times.txt...')
trip_stops = {}
used_stops = set()
n = 0
st_match = next((nm for nm in names if nm.split('/')[-1].lower() == 'stop_times.txt'), None)
...
stops_by_id = {}
parent_of   = {}
rd, _ = find_and_read(zf, 'stops.txt')
for row in rd:
    sid = row.get('stop_id', '')
    if not sid: continue
    stops_by_id[sid] = row
...
CREATE TABLE stops (stop_id TEXT PRIMARY KEY, stop_name TEXT NOT NULL,
  stop_lat REAL, stop_lon REAL, country_code TEXT DEFAULT "NL",
...
for sid in needed:
    ...
    stop_rows.append((sid, row.get('stop_name',''), lat, lon, 'NL', lt, row.get('parent_station','')))
...
c2.execute("""
    ...
    WHERE (s_o.stop_name LIKE '%Amsterdam%') AND (s_d.stop_name LIKE '%Rotterdam%')
      AND st_o.departure_time >= '08:00:00'
    LIMIT 5
""")
print('Sample Amsterdam→Rotterdam:', c2.fetchall())
con2.close()
