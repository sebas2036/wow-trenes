#!/usr/bin/env python3
"""
WoW TRENES - Importador GTFS Alemania -> SQLite
Lee ~/Downloads/Wow trains Germany/ y genera assets/gtfs_germany.db

Fuente: gtfs.de — Schienenfernverkehr Deutschland (ICE + IC)
  URL:     https://gtfs.de/de/feeds/de_fv
  ZIP:     https://download.gtfs.de/germany/fv_free/latest.zip
  Licencia: Creative Commons 4.0
  Base:    DELFI NeTEx dataset (datos abiertos nacionales de Alemania)

Incluye: ICE, IC, EC, Nightjet (DB Fernverkehr)
No incluye: S-Bahn, U-Bahn, Regionalexpress (feeds separados)

Resultado esperado: ~1.300 estaciones, ~30-50k stop_times, ~5-10 MB
"""
import sqlite3, csv, os, sys, time
from pathlib import Path

# Acepta varios nombres posibles de carpeta
_POSSIBLE_DIRS = [
    Path.home() / "Downloads" / "Wow trains Germany",
    Path.home() / "Downloads" / "Alemania train",
    Path.home() / "Downloads" / "Alemania Train",
    Path.home() / "Downloads" / "ALEMANIA TRAIN",
    Path.home() / "Downloads" / "ALEMANIA TRAIN ",
    Path.home() / "Downloads" / "germany_fv",
    Path.home() / "Downloads" / "latest",
    Path.home() / "Downloads" / "fv_free",
]
GTFS_DIR   = next((d for d in _POSSIBLE_DIRS if d.exists()), _POSSIBLE_DIRS[0])
OUTPUT_DIR = Path(__file__).parent.parent / "assets"
OUTPUT_DB  = OUTPUT_DIR / "gtfs_germany.db"
COUNTRY    = "DE"
...
def setup(conn):
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS stops (
            ...
            country_code TEXT DEFAULT 'DE', location_type INTEGER DEFAULT 0,
            parent_station TEXT);
...
def main():
    print("\nWoW TRENES - Importador GTFS Alemania (DB Fernverkehr)")
    print("=" * 52)
    ...
    if not GTFS_DIR.exists():
        ...
        print("\nPasos:")
        print("  1. Baja: https://download.gtfs.de/germany/fv_free/latest.zip")
        print("  2. Descomprime en: ~/Downloads/Wow trains Germany/")
        sys.exit(1)
    ...
    try:
        ...
        conn.executemany("INSERT OR IGNORE INTO agency VALUES (?,?,?,?)",
            [(r.get("agency_id",""), r.get("agency_name",""),
              r.get("agency_url",""), r.get("agency_timezone","Europe/Berlin"))
             for r in rows])
        conn.commit()
        print(f"  Agencias: {len(rows)}")

        # Routes — incluir todas si no hay tipos rail estándar
        ...
        print("\nAlemania lista con datos reales de DB Fernverkehr (ICE + IC).")
