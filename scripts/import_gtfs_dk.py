#!/usr/bin/env python3
"""
WoW TRENES - Importador GTFS Dinamarca -> SQLite
Lee ~/Downloads/Wow trains Denmark/ y genera assets/gtfs_dk.db

Fuente: Rejseplanen — desconocida (Rejseplanen Labs)
  URL: https://www.rejseplanen.info/labs/GTFS.zip
  Descarga directa — Rejseplanen Labs (operador nacional danés).

Incluye: Rejseplanen nacional — København, Aarhus, Odense, Aalborg

Resultado esperado: ~400-600 estaciones rail, ~40-80k stop_times, ~10-20 MB
"""
import sqlite3, csv, sys, time, shutil
from pathlib import Path

_POSSIBLE_DIRS = [
    Path.home() / "Downloads" / "Wow trains Denmark",
    Path.home() / "Downloads" / "Wow trains Denmark ",
    Path.home() / "Downloads" / "DENMARK",
    Path.home() / "Downloads" / "Denmark",
    Path.home() / "Downloads" / "DINAMARCA",
    Path.home() / "Downloads" / "rejseplanen",
    Path.home() / "Downloads" / "DSB",]
GTFS_DIR   = next((d for d in _POSSIBLE_DIRS if d.exists()), _POSSIBLE_DIRS[0])
OUTPUT_DIR = Path(__file__).parent.parent / "assets"
OUTPUT_DB  = OUTPUT_DIR / "gtfs_dk.db"
COUNTRY    = "DK"
...
def setup(conn):
    conn.executescript("""
        ...
        CREATE TABLE IF NOT EXISTS stops (
            ...
            country_code TEXT DEFAULT 'DK', location_type INTEGER DEFAULT 0,
            parent_station TEXT);
...
def main():
    print(f"\nWoW TRENES - Importador GTFS Dinamarca (Rejseplanen)")
    print("=" * 55)
    ...
    if not GTFS_DIR.exists():
        ...
        print("\nDescarga:")
        print("  https://www.rejseplanen.info/labs/GTFS.zip")
        print("  Descarga directa — Rejseplanen Labs (operador nacional danés).")
        print(f"  Descomprime en: ~/Downloads/Wow trains Denmark/")
        sys.exit(1)
    ...
    tmp_db = Path("/tmp/gtfs_dk_build.db")
    ...
    try:
        ...
        conn.executemany("INSERT OR IGNORE INTO agency VALUES (?,?,?,?)",
            [(r.get("agency_id",""), r.get("agency_name",""),
              r.get("agency_url",""), r.get("agency_timezone","Europe/Copenhagen"))
             for r in rows])
        conn.commit()
        ...
        print(f"\nDinamarca lista con datos de Rejseplanen.")
