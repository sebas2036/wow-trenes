#!/usr/bin/env python3
"""
Importa GTFS real Trenitalia → assets/gtfs_italy.db
Acepta .zip o .gtfs (mismo formato, distinta extensión).

Uso:
  python3 scripts/import_gtfs_italy_real.py <ruta_al_archivo>

Ejemplo:
  python3 scripts/import_gtfs_italy_real.py ~/Downloads/trenitalia.gtfs

Filtra: route_type IN (2,100,101,102,103,104,105,107) — Frecciarossa, Frecciargento, IC
Excluye: 106/109 (regional lento), 700+ (bus)
"""

import csv, io, os, sqlite3, sys, time, zipfile

SRC_PATH = sys.argv[1] if len(sys.argv) > 1 else 'trenitalia.gtfs'
OUT_DB   = os.path.normpath(os.path.join(os.path.dirname(__file__), '..', 'assets', 'gtfs_italy.db'))
...
def zreader(zf, base):
    ...
    if not match:
        raise FileNotFoundError(f'{base} not found in zip. Available: {names[:10]}')
    data = zf.read(match).decode('utf-8-sig')
    return csv.DictReader(io.StringIO(data))

log(f'=== Importando GTFS Italia (Trenitalia) desde {SRC_PATH} ===')
...
if not rail_routes:
    log('  → Sin filtro de tipo (importando todas las rutas)')
    for row in zreader(zf, 'routes.txt'):
        try:
            rt = int(row.get('route_type', 2))
        except ValueError:
            rt = 2
        rail_routes[row['route_id']] = {
            'short':  row.get('route_short_name', ''),
...
used_stops = set()
n = 0
st_file = next((nm for nm in names if nm.split('/')[-1].lower() == 'stop_times.txt'), None)
with zf.open(st_file) as raw:
...
CREATE TABLE stops (stop_id TEXT PRIMARY KEY, stop_name TEXT NOT NULL,
  stop_lat REAL, stop_lon REAL, country_code TEXT DEFAULT "IT",
...
for sid in needed:
    ...
    stop_rows.append((sid, row.get('stop_name',''), lat, lon, 'IT', lt, row.get('parent_station','')))
...
c2.execute("""
    ...
    WHERE (s_o.stop_name LIKE '%Milano%' OR s_o.stop_name LIKE '%Roma%')
      AND (s_d.stop_name LIKE '%Roma%' OR s_d.stop_name LIKE '%Napoli%')
      AND st_o.departure_time >= '08:00:00'
    LIMIT 5
""")
print('Sample Milano/Roma:', c2.fetchall())
con2.close()
