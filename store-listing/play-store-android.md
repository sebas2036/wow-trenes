# WoW Train — Google Play Console (Android)
# Pegar estos textos en Play Console → tu app → Store listing

---

## TITLE (50 chars max)
WoW Train: Trenes Europa Sin Cuenta

## SHORT DESCRIPTION (80 chars max)
GPS detecta tu estación. Compra en 3 toques. QR offline. Sin contraseña.

## FULL DESCRIPTION (4000 chars max)
WoW Train es la app de viajes en tren más rápida de Europa. Sin registrarte, sin crear cuenta — solo abre la app, y el GPS detecta automáticamente en qué país estás y qué trenes salen desde tu estación más cercana.

🚄 COMPRA EN 3 TOQUES
Elige el tren → selecciona clase → paga con Google Pay. Tu QR aparece al instante, guardado offline en el teléfono. Sin señal en el andén, sin problema.

📍 GEOFENCING INTELIGENTE
La app te avisa cuando estás a 1 km de la estación. Cuando llegas a 50 metros, el QR se abre automáticamente — pasas el torniquete sin tocar el teléfono.

🗺️ MAPA EN TIEMPO REAL
Ve exactamente dónde está tu tren mientras esperas en el andén. Retrasos actualizados para:
• 🇨🇭 SBB (Suiza)
• 🇳🇱 NS (Países Bajos)
• 🇮🇹 Trenitalia (Italia)
• 🇫🇷 SNCF (Francia)
• 🇩🇪 DB / ICE (Alemania)
• 🚇 Metro Madrid, Barcelona, NYC, Chicago, LA, Londres, Tokio

🌍 TUS DESTINOS
Ferrocarriles soportados: SBB · SNCF · DB · Renfe · Trenitalia · NS · ÖBB · National Rail · Amtrak · JR · CRH y muchos más.

📷 TRADUCTOR POR FOTO
¿Un cartel en japonés? ¿Una pantalla en alemán? Toma una foto y la app traduce al instante — sin salir de la aplicación, sin conexión adicional.

🔒 PRIVACIDAD POR DISEÑO
• Sin cuenta de usuario
• Sin contraseña que recordar
• Los billetes se guardan solo en tu teléfono
• El pago lo gestiona Trainline (Merchant of Record) — WoW Train nunca ve tu tarjeta
• Cumple RGPD Art. 5 — mínimo de datos

📅 PLANIFICADOR DE DÍA
Dile a la app a dónde vas y te sugiere el mejor tren para cada punto de interés — museos, restaurantes, hoteles — calculado desde tu ubicación en tiempo real.

Descarga WoW Train y viaja por Europa como un local. Tu tren. Tu momento.

---

## CATEGORY
Travel & Local

## TAGS (Play Store — máx 5)
tren, train, europe, ticket, rail

## EMAIL (contacto en la store)
jasinskysebastian@gmail.com

## PRIVACY POLICY URL (obligatorio)
https://wowtrenes.app/privacy

## WEBSITE (opcional)
https://wowtrenes.app

---

## CONTENT RATING QUESTIONNAIRE
• Violence: No
• Sexuality: No
• Language: No
• Controlled substances: No
• User-generated content: No
→ Rating esperada: Everyone (PEGI 3 / ESRB Everyone)

---

## DATA SAFETY (obligatorio desde 2022)

**Location:**
- Approximate location: YES (detectar país/estación cercana)
- Precise location: YES (geofencing Ring 1/2/3)
- Collected: YES | Shared: NO | Required: YES
- Purpose: App functionality

**Financial info (tarjeta de crédito):**
- NOT collected by WoW Train (procesado por Trainline externamente)

**App activity:**
- NOT collected

**Device or other IDs:**
- NOT collected

---

## WHAT'S NEW — v1.0
Primera versión de WoW Train.
• GPS detecta tu país y estación más cercana automáticamente
• Compra con Google Pay en 3 toques — QR guardado offline
• Geofencing: el QR se abre solo al entrar a la estación
• Mapa en tiempo real: SBB, NS, Trenitalia, SNCF, DB
• Traductor por foto integrado
• Sin cuenta. Sin contraseña.

---

## CAPTURAS DE PANTALLA RECOMENDADAS (Phone: 1080×2400)

1. Home — Mapa con country cards de destinos europeos
2. Split-screen — Reloj regresivo + mapa con posición del tren
3. Checkout — Sheet de compra con clase y botón Google Pay
4. QR Ticket — Overlay del QR con datos del viaje
5. Notificaciones — Alerta "Tu tren llega al andén 3 en 8 min"
6. Traductor — Foto de cartel → traducción

## FEATURE GRAPHIC (1024×500 — obligatorio)
Fondo degradado violeta → negro (#7C3AED → #09090B)
Logo "WoW Train" centrado en blanco
Tagline: "Tu tren. Tu momento." en lavanda (#C4B5FD)
